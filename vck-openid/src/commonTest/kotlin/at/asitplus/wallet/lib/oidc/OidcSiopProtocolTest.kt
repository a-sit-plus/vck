...
class OidcSiopProtocolTest : FreeSpec({
    ...
    "test with QR Code" {
        ...
        val jar = authnRequest.request
            .shouldNotBeNull()
        val jwsObject = JwsSigned.deserialize<AuthenticationRequestParameters>(jar, AuthenticationRequestParameters.serializer(), vckJsonSerializer).getOrThrow()
    ...
    "test with direct_post_jwt" {
        ...
        val jwsObject = JwsSigned.deserialize<AuthenticationResponseParameters>(jarmResponse, AuthenticationResponseParameters.serializer()).getOrThrow()
        DefaultVerifierJwsService().verifyJwsObject(jwsObject).shouldBeTrue()
...
private fun attestationJwtVerifier(trustedKey: JsonWebKey) =
    object : RequestObjectJwsVerifier {
        override fun invoke(jws: JwsSigned<RequestParameters>): Boolean {
            val attestationJwt = jws.header.attestationJwt?.let { JwsSigned.deserialize<JsonWebToken>(it, JsonWebToken.serializer()).getOrThrow() }
                ?: return false

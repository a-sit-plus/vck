...
val OidvciCodeFlowTest by testSuite {
    ...
    withFixtureGenerator {
        ...
        "request one credential, using scope" { it ->
        ...
        "request multiple credentials, using scope" { it ->
        ...
        "proof over different keys leads to different credentials" { it ->
            val requestOptions = RequestOptions(AtomicAttribute2023, PLAIN_JWT)
        ...
        "authorizationService with defect mapstore leads to an error" { it ->
        ...
        "request credential in SD-JWT, using scope" { it ->
        ...
        "request credential in SD-JWT, using scope only in authn request" { it ->
            val requestOptions = RequestOptions(AtomicAttribute2023, SD_JWT)
            val credentialFormat = it.client.selectSupportedCredentialFormat(requestOptions, it.issuer.metadata)
                .shouldNotBeNull()
            val scope = credentialFormat.scope.shouldNotBeNull()
        ...
        "request credential in SD-JWT, using scope in access token different to auth code" { it ->
        ...
        "request credential in SD-JWT, using authorization details" { it ->
        ...
        "request credential in SD-JWT, using authorization details only in authnrequest" { it ->
            val credentialConfigurationId = AtomicAttribute2023.toCredentialIdentifier(SD_JWT)
            val authorizationDetails = it.client.buildAuthorizationDetails(
                credentialConfigurationId = credentialConfigurationId,
                authorizationServers = it.issuer.metadata.authorizationServers
        ...
        "request credential in SD-JWT, using authorization details in access token different to auth code" { it ->
        ...
        "request credential in SD-JWT, using more authorization details in access token than in auth code" { it ->
            val authCodeAuthnDetails = it.client.buildAuthorizationDetails(
                credentialConfigurationId = AtomicAttribute2023.toCredentialIdentifier(SD_JWT),
                authorizationServers = it.issuer.metadata.authorizationServers
            )

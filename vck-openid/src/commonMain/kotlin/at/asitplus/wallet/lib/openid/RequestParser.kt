...
class RequestParser(
    ...
    private fun DCAPIWalletRequest.OpenId4Vp.parseAsDcApiRequest(): RequestParametersFrom<*>? = catchingUnwrapped {
        when (this) {
            is DCAPIWalletRequest.OpenId4VpSigned -> {
                ...
                val jwsSigned = vckJsonSerializer.decodeFromString(JwsCompact.JwsCompactSerializer, requestStr)
                RequestParametersFrom.DcApiSigned(this, jwsSigned.getPayload(), jwsSigned)
            }
    ...
    private suspend fun String.parseAsJwsRequest(
        ...
        vckJsonSerializer.decodeFromString(JwsCompact.JwsCompactSerializer, this).let {
            RequestParametersFrom.JwsSigned(
                it,
                it.getPayload(RequestParameters.serializer(), vckJsonSerializer),
                verified = false, //requestObjectJwsVerifier.invoke(it),

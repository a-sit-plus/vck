...
internal class PresentationFactory(
    ...
    private fun parseTransactionData(request: RequestParameters): Pair<PresentationRequestParameters.Protocol, Collection<TransactionData>>? {
        ...
        val (protocol, keysToDecode) = if (decoded.values.any { it.contains("credential_ids") }) {
            PresentationRequestParameters.Protocol.OID4VP to decoded.filterValues { it.contains("credential_ids") }.keys
        } else {
            PresentationRequestParameters.Protocol.UC5 to decoded.keys
        }
        ...
        return protocol to transactionData
    }

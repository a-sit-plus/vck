...
val SdJwtTypeMetadata16SerializationTest by testSuite {
                    ...
                    get() = claims?.map {
                        ClaimDescription(
                            path = OpenId4VciClaimsPathPointer(it.path.map {
                                when (it) {
                                    is DCQLClaimsPathPointerSegment.IndexSegment -> OpenId4VciClaimsPathPointerSegmentIndex(
                                        it.index
                                    )

                                    is DCQLClaimsPathPointerSegment.NameSegment -> OpenId4VciClaimsPathPointerSegmentString(
                                        it.name
                                    )
                            ...
                            display = it.display?.map {
                                val renderData = it.rendering?.first()
                                DisplayProperties(
                                    locale = it.language,
                                    name = it.language,
                                    description = it.description,
                                    textColor = renderData?.textColor,
                                    backgroundColor = renderData?.backgroundColor,

                                    // TODO: more recent sd jwt type metadata has more display properties
                                )
                            }?.toSet() ?: setOf()
                        )
                    } ?: setOf()

        ...
        val statusResolver = StatusListTokenResolver.FromHttpRequestResolver(
            ...
            when (mediaType) {
                StatusListTokenMediaType.Jwt -> {
                    ...
                    WebToken.JsonWebToken(jwsSigned)
                }

                StatusListTokenMediaType.Cwt -> {
                    val coseSigned = CoseSigned.deserialize(
                        CborWebToken.serializer(),
                        byteArray
                    ).getOrThrow()
                    WebToken.CborWebToken(coseSigned)
                }
            }
        }

        withData("jwt", "cwt") {
            statusResolver.invoke(
                UniformResourceIdentifier("https://www.example.com/$it")
            )
        }
    }
})